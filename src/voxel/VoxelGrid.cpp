...
VoxelGrid::VoxelGrid(Bounds bounds, float resolution) 
...
size_t VoxelGrid::getIndex(int x, int y, int z) const {
    return static_cast<size_t>(x) * m_dimY * m_dimZ + static_cast<size_t>(y) * m_dimZ + z;
}

bool VoxelGrid::isSolid(int x, int y, int z) const {
    if (x < 0 || x >= m_dimX || y < 0 || y >= m_dimY || z < 0 || z >= m_dimZ) return false;
    return m_data[getIndex(x, y, z)] == 1;
}

uint64_t VoxelGrid::subtractCylinder(Vector3 center, float radius, float height) {
    ...
    for (int x = minX; x <= maxX; ++x) {
        for (int y = minY; y <= maxY; ++y) {
            float dx = (x * m_resolution + m_bounds.minX) - center.x;
            float dy = (y * m_resolution + m_bounds.minY) - center.y;
